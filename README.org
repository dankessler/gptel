...
SuperGrok subscribers can use their subscription without an xAI API key by
registering an OAuth backend:

#+begin_src emacs-lisp
(gptel-make-xai-oauth "SuperGrok") ; Any name of your choosing
#+end_src

The first request opens xAI's login page in your browser.  By default, the OAuth
access token, refresh token, and expiry are stored in
=.cache/gptel-xai/xai-oauth-token= under =user-emacs-directory= (typically
=~/.emacs.d/.cache/gptel-xai/xai-oauth-token=).  Customize
=gptel-xai-oauth-token-file= to use another location.

You can authenticate explicitly with =M-x gptel-xai-oauth-login= and delete the
locally stored credentials with =M-x gptel-xai-oauth-logout=.  Logging out does
not revoke the grant at xAI; the next request starts a new login.  The browser
must be running on the same machine as Emacs so that it can reach the temporary
localhost callback.

To make it the default backend, use:

#+begin_src emacs-lisp
(setq gptel-model 'grok-4.5
      gptel-backend (gptel-make-xai-oauth "SuperGrok"))
#+end_src
...
| =gptel-xai-oauth-login=  | Authenticate with xAI (SuperGrok). (Automatically handled, but can be forced if required.) |
| =gptel-xai-oauth-logout= | Delete locally stored xAI (SuperGrok) OAuth credentials.                                  |
|----------------------------+-------------------------------------------------------------------------------------------|
