...
The first request opens xAI's login page in your browser and stores the OAuth
token under =user-emacs-directory=.  You can also authenticate explicitly with
=M-x gptel-xai-oauth-login=.  The browser must be running on the same machine as
Emacs so that it can reach the temporary localhost callback.
